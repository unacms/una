...
DELETE `sys_localization_strings` FROM `sys_localization_strings`, `sys_localization_keys` WHERE `sys_localization_keys`.`ID` = `sys_localization_strings`.`IDKey` AND `sys_localization_keys`.`Key` IN('_bx_files_fav_failed');
DELETE FROM `sys_localization_keys` WHERE `Key` IN('_bx_files_fav_failed');

UPDATE `sys_modules` SET `version` = '1.0.5' WHERE `uri` = 'files' AND `version` = '1.0.4';
