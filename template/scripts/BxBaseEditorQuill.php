...
class BxBaseEditorQuill extends BxDolEditor
{
    ...
    protected static $CONF_COMMON = <<<EOS
                    quill = new Quill(
                        '{bx_var_selector}',
                         {
                         placeholder: 'Compose an epic...', theme: '{bx_var_skin}',
                         modules: {toolbar: [{bx_var_plugins}]}
                         }
                    );          
    EOS;
   /* protected static $CONF_COMMON = <<<EOS
...
*/
      ...
      /*      'bx_var_custom_init' => $sCustomInit,
        ...
        if ($bDynamicMode) {
            ...
            $sScript = "<script>
                if ('undefined' == typeof(jQuery(document).quill)) {
                    window.quillPreInit = {base : '" . bx_js_string(BX_DOL_URL_PLUGINS_PUBLIC . 'quill', BX_ESCAPE_STR_APOS) . "', suffix : '.min', query : ''};
                    $.getScript('" . bx_js_string(BX_DOL_URL_ROOT . 'inc/js/editor.quill.js', BX_ESCAPE_STR_APOS) . "');
                    $.getScript('" . bx_js_string(BX_DOL_URL_PLUGINS_PUBLIC . 'quill/quill.min.js', BX_ESCAPE_STR_APOS) . "', function(data, textStatus, jqxhr) {});
                } else {
                	setTimeout(function () {
                    	$sInitEditor
    ...
    protected function _addJsCss($bDynamicMode = false, $sInitEditor = '')
    {
        ...
        if ($this->_bJsCssAdded)
            return '';
        $this->_oTemplate->addJs(array('quill/quill.min.js'));
        $this->_oTemplate->addCss(BX_DIRECTORY_PATH_PLUGINS_PUBLIC . 'quill/|quill.' . $this->_aObject['skin'] . '.css');
