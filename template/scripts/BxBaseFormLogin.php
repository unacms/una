...
class BxBaseFormLogin extends BxTemplFormView {
    ...
    function isValid () {
        if (!parent::isValid ()) {
            $this->_setLoginError (_t('_sys_txt_login_error'));
            return false;
        }
    ...
    function isSubmittedAndValid () {
        $bRet = parent::isSubmittedAndValid ();
        if (!$bRet && !$this->getLoginError())
            $this->_setLoginError (_t('_sys_txt_login_error'));
        return $bRet;
    }
