...
-- PRIVACY 
DELETE FROM `sys_objects_privacy` WHERE `object` = 'bx_reviews_allow_view_to';
...
-- SEARCH
DELETE FROM `sys_objects_search` WHERE `ObjectName` IN ('bx_reviews', 'bx_reviews_cmts');
...
-- CRON
DELETE FROM `sys_cron_jobs` WHERE `name` LIKE 'bx_reviews%';
