...
DELETE FROM `sys_options` WHERE `name`='bx_files_enable_auto_approve';
INSERT INTO `sys_options` (`name`, `value`, `category_id`, `caption`, `type`, `check`, `check_params`, `check_error`, `extra`, `order`) VALUES
('bx_files_enable_auto_approve', 'on', @iCategId, '_bx_files_option_enable_auto_approve', 'checkbox', '', '', '', '', 0);


-- PAGES
DELETE FROM `sys_pages_blocks` WHERE `object`='bx_files_view_entry' AND `title`='_bx_files_page_block_title_entry_reports';
...
('bx_files_view_entry', 2, 'bx_files', '', '_bx_files_page_block_title_entry_reports', 11, 2147483647, 'service', 'a:2:{s:6:\"module\";s:8:\"bx_files\";s:6:\"method\";s:14:\"entity_reports\";}', '', 0, '', 0, 0, 1, 6);

UPDATE `sys_pages_blocks` SET `title_system`='_bx_files_page_block_title_sys_favorites_entries' WHERE `object`='bx_files_favorites' AND `title`='_bx_files_page_block_title_favorites_entries';


-- MENUS
DELETE FROM `sys_menu_items` WHERE `set_name`='bx_files_view' AND `name`='approve'; 
...
DELETE FROM `sys_menu_items` WHERE `set_name`='bx_files_view_actions' AND `name` IN ('approve', 'notes', 'audit'); 
...
UPDATE `sys_objects_menu` SET `template_id`='15' WHERE `object`='bx_files_view_inline';


-- ACL
SET @iIdActionEntryDeleteAny = (SELECT `ID` FROM `sys_acl_actions` WHERE `Module`='bx_files' AND `Name`='delete any entry' LIMIT 1);
...
-- GRIDS
UPDATE `sys_grid_fields` SET `width`='15%' WHERE `object`='bx_files_common' AND `name`='added';

DELETE FROM `sys_grid_fields` WHERE `object`='bx_files_common' AND `name`='status_admin';
