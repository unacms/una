...
-- FORMS
DELETE FROM `sys_form_inputs` WHERE `object`=@sName AND `name`='is_track_links';
...
DELETE FROM `sys_form_display_inputs` WHERE `display_name` IN ('bx_massmailer_campaign_add', 'bx_massmailer_campaign_edit') AND `input_name`='is_track_links';
INSERT INTO `sys_form_display_inputs` (`display_name`, `input_name`, `visible_for_levels`, `active`, `order`) VALUES
('bx_massmailer_campaign_add', 'is_track_links', 2147483647, 1, 7),
('bx_massmailer_campaign_edit', 'is_track_links', 2147483647, 1, 7);
