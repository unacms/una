...
DELETE FROM `sys_pages_blocks` WHERE `module` = 'bx_videos' OR `object` IN('bx_videos_create_entry', 'bx_videos_edit_entry', 'bx_videos_delete_entry', 'bx_videos_view_entry', 'bx_videos_view_entry_comments', 'bx_videos_home', 'bx_videos_popular', 'bx_videos_updated', 'bx_videos_author', 'bx_videos_search', 'bx_videos_manage');


-- MENU
DELETE FROM `sys_objects_menu` WHERE `module` = 'bx_videos';
DELETE FROM `sys_menu_sets` WHERE `module` = 'bx_videos';
DELETE FROM `sys_menu_items` WHERE `module` = 'bx_videos' OR `set_name` IN('bx_videos_view', 'bx_videos_submenu', 'bx_videos_view_submenu', 'bx_videos_my');
