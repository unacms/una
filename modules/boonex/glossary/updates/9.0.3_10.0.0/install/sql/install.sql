-- TABLES
ALTER TABLE `bx_glossary_terms` CHANGE `allow_view_to` `allow_view_to` VARCHAR(16) NOT NULL DEFAULT '3';
...
-- FORMS
DELETE FROM `sys_form_inputs` WHERE `object`='bx_glossary' AND `name`='labels';
...
-- VOTES
DELETE FROM `sys_objects_vote` WHERE `Name`='bx_glossary_reactions';
