...
DELETE FROM `sys_options` WHERE `name` IN ('bx_glossary_enable_auto_approve', 'bx_glossary_activate_terms_after_creation');
INSERT INTO `sys_options` (`name`, `value`, `category_id`, `caption`, `type`, `check`, `check_params`, `check_error`, `extra`, `order`) VALUES
('bx_glossary_enable_auto_approve', 'on', @iCategId, '_bx_glossary_option_enable_auto_approve', 'checkbox', '', '', '', '', 0);


-- PAGES
DELETE FROM `sys_pages_blocks` WHERE `object`='bx_glossary_view_entry' AND `title`='_bx_glossary_page_block_title_entry_reports';
...
('bx_glossary_view_entry', 2, 'bx_glossary', '', '_bx_glossary_page_block_title_entry_reports', 11, 2147483647, 'service', 'a:2:{s:6:\"module\";s:11:\"bx_glossary\";s:6:\"method\";s:14:\"entity_reports\";}', '', 0, '', 0, 0, 1, 6);

UPDATE `sys_pages_blocks` SET `title_system`='_bx_glossary_page_block_title_sys_favorites_entries' WHERE `object`='bx_glossary_favorites' AND `title`='_bx_glossary_page_block_title_favorites_entries';


-- MENUS
DELETE FROM `sys_menu_items` WHERE `set_name`='bx_glossary_view' AND `name`='approve';
...
DELETE FROM `sys_menu_items` WHERE `set_name`='bx_glossary_view_actions' AND `name` IN ('approve', 'notes', 'audit');
...
-- GRIDS
UPDATE `sys_grid_fields` SET `width`='15%' WHERE `object`='bx_glossary_common' AND `name`='added';

DELETE FROM `sys_grid_fields` WHERE `object`='bx_glossary_common' AND `name`='status_admin';
