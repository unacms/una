-- PAGES
DELETE FROM `sys_objects_page` WHERE `object`='bx_glossary_top';
...
DELETE FROM `sys_pages_blocks` WHERE `object`='bx_glossary_top';
...
-- MENUS
DELETE FROM `sys_menu_items` WHERE `set_name`='bx_glossary_submenu' AND `name`='glossary-top';
...
--GRIDS
DELETE FROM `sys_grid_actions` WHERE `object`='bx_glossary_administration' AND `name`='audit_content';
