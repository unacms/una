-- PAGES
DELETE FROM `sys_pages_blocks` WHERE `object`='' AND `module`='bx_persons' AND `title_system`='_bx_persons_page_block_title_sys_active_profiles';
...
-- MENUS
DELETE FROM `sys_menu_items` WHERE `set_name`='bx_persons_view_actions' AND `name`='profile-set-badges';
...
DELETE FROM `sys_menu_items` WHERE `set_name`='bx_persons_view_actions_all' AND `name`='profile-set-badges';
INSERT INTO `sys_menu_items`(`set_name`, `module`, `name`, `title_system`, `title`, `link`, `onclick`, `target`, `icon`, `addon`, `submenu_object`, `submenu_popup`, `visible_for_levels`, `visibility_custom`, `active`, `copyable`, `order`) VALUES 
...
-- GRIDS
DELETE FROM `sys_grid_actions` WHERE `object`='bx_persons_administration' AND `name` IN ('audit_content', 'audit_profile');
...
-- PRIVACY
UPDATE `sys_objects_privacy` SET `default_group`='5' WHERE `object`='bx_persons_allow_post_to';

DELETE FROM `sys_objects_privacy` WHERE `object`='bx_persons_allow_contact_to';
