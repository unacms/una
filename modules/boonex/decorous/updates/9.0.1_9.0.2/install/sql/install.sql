...
SET @iCategoryId = (SELECT `id` FROM `sys_options_categories` WHERE `name`=CONCAT(@sName, '_styles_menu_add') LIMIT 1);
DELETE FROM `sys_options` WHERE `name`=CONCAT(@sName, '_menu_add_bg_image_attachment');
INSERT INTO `sys_options`(`category_id`, `name`, `caption`, `value`, `type`, `extra`, `check`, `check_error`, `order`) VALUES
