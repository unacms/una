-- SETTINGS
SET @iCategId = (SELECT `id` FROM `sys_options_categories` WHERE `name`='bx_tasks' LIMIT 1);

DELETE FROM `sys_options` WHERE `name`='bx_tasks_enable_auto_approve';
INSERT INTO `sys_options` (`name`, `value`, `category_id`, `caption`, `type`, `check`, `check_params`, `check_error`, `extra`, `order`) VALUES
('bx_tasks_enable_auto_approve', 'on', @iCategId, '_bx_tasks_option_enable_auto_approve', 'checkbox', '', '', '', '', 0);


-- PAGES
DELETE FROM `sys_pages_blocks` WHERE `object`='bx_tasks_view_entry' AND `title`='_bx_tasks_page_block_title_entry_reports';
...
('bx_tasks_view_entry', 2, 'bx_tasks', '', '_bx_tasks_page_block_title_entry_reports', 11, 2147483647, 'service', 'a:2:{s:6:\"module\";s:8:\"bx_tasks\";s:6:\"method\";s:14:\"entity_reports\";}', '', 0, '', 0, 0, 1, 6);


-- MENUS
DELETE FROM `sys_menu_items` WHERE `set_name`='bx_tasks_view' AND `name`='approve';
...
DELETE FROM `sys_menu_items` WHERE `set_name`='bx_tasks_view_actions' AND `name` IN ('approve', 'audit');
...
-- ACL
SET @iIdActionEntryDeleteAny = (SELECT `ID` FROM `sys_acl_actions` WHERE `Module`='bx_tasks' AND `Name`='delete any entry' LIMIT 1);
