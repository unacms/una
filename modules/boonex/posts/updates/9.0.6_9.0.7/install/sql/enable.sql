-- SETTINGS
UPDATE `sys_options` SET `value`='6' WHERE `name`='bx_posts_per_page_profile';

SET @iCategId = (SELECT `id` FROM `sys_options_categories` WHERE `name`='bx_posts' LIMIT 1);
DELETE FROM `sys_options` WHERE `name`='bx_posts_per_page_browse_showcase';
INSERT INTO `sys_options` (`name`, `value`, `category_id`, `caption`, `type`, `extra`, `check`, `check_error`, `order`) VALUES
('bx_posts_per_page_browse_showcase', '32', @iCategId, '_sys_option_per_page_browse_showcase', 'digit', '', '', '', 15);


-- PAGES
DELETE FROM `sys_pages_blocks` WHERE `module`='bx_posts' AND `title` IN ('_bx_posts_page_block_title_recent_entries_view_showcase', '_bx_posts_page_block_title_popular_entries_view_showcase', '_bx_posts_page_block_title_featured_entries_view_showcase');
...
-- METATAGS
UPDATE `sys_objects_metatags` SET `table_mentions`='bx_posts_meta_mentions' WHERE `object`='bx_posts';
